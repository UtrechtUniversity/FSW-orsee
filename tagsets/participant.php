...
function participant__exclude_participant($participant)
{
    ...
    $pars = array(
        ':status_id' => $settings['automatic_exclusion_to_participant_status'],
        ':deletion_time' => time(),
        ':remarks' => $remarks,
    ...
    while ($p=pdo_fetch_assoc($result)) {
        ...
        if ($p['sess_id'] != '0' || true) { //$last_reg_time > $now) {
            ...
            */
            ...
            if ($p['pstatus_id'] > 0) {
                echo '<FONT color="';
                if ($pstatuses[$p['pstatus_id']]['noshow']) echo $color['shownup_no'];
                else echo $color['shownup_yes'];
                echo '">';
            }
...
function participant__create_participant_id($pdata = array())
{
    ...
    while ($exists) {
        ...
        $line = orsee_query($query, $pars);
        if (isset($line['participant_id'])) $exists = true;
        else $exists = false;
    }
...
function form__render_field($f)
{
    ...
    switch ($f['type']) {
        case 'textline':
            $out = form__render_textline($f);
            break;
        case 'textarea':
            $out = form__render_textarea($f);
            break;
        case 'radioline':
            $out = form__render_radioline($f);
            break;
        case 'select_list':
            $out = form__render_select_list($f);
            break;
        case 'select_numbers':
            $out = form__render_select_numbers($f);
            break;
        case 'select_lang':
            $out = form__render_select_lang($f);
            break;
        case 'radioline_lang':
            $out = form__render_radioline_lang($f);
            break;
    }
    return $out;
}
...
function form__render_radioline($f)
{
    ...
    foreach ($items as $val => $text) {
        ...
        $out .= '>';
        if (isset($lang[$text])) $out .= $lang[$text];
        else $out .= $text;
        $out .= '&nbsp;&nbsp;&nbsp;';
    }
    return $out;
...
function form__render_select_list($f, $formfieldvarname = '')
{
    ...
    $optionvalues = explode(",", $f['option_values']);
    $optionnames = explode(",", $f['option_values_lang']);
    if ($f['include_none_option'] == 'y') $incnone = true;
    else $incnone = false;
    $items = array();
    foreach ($optionvalues as $k => $v) {
        if (isset($optionnames[$k])) $items[$v] = $optionnames[$k];
...
function form__render_select_numbers($f)
{
    ...
    if ($f['values_reverse'] == 'y') $reverse = true;
    else $reverse = false;
...
function participant__select_numbers($ptablevarname, $formfieldvarname, $prevalue, $begin, $end, $fillzeros = 2, $steps = 1, $reverse = false, $incnone = false, $existing = false, $where = '', $show_count = false)
{
    ...
    if (!$existing) {
        ...
        foreach ($listitems as $line) {
            ...
            if ($line['tf_value'] == (int) $prevalue) $out .= ' SELECTED';
            $out .= '>';
...
function participant__select_existing($ptablevarname, $formfieldvarname, $prevalue, $where = '', $show_count = false)
{
    ...
    while ($line = pdo_fetch_assoc($result)) {
        $out .= '<option value="' . $line['tf_value'] . '"';
        if ($line['tf_value'] == $prevalue) $out .= ' SELECTED';
        $out .= '>';
        $out .= $line['tf_value'];
        if ($show_count) $out .= ' (' . $line['tf_count'] . ')';
        $out .= '</option>';
...
function participant__show_admin_form($edit,$button_title="",$errors=array(),$extra="") {
    ...
    if (isset($edit['creation_time'])) $tout['creation_time'] = ortime__format($edit['creation_time'], '', lang('lang'));
    else $tout['creation_time'] = '';
    ...
    echo '  <tr><td>' . lang('id') . '</td>
        ...
        <tr><td>' . lang('creation_time') . '</td>
            <td>';
    if (isset($edit['creation_time'])) echo ortime__format($edit['creation_time'], '', lang('lang'));
    else echo '???';
    echo '  </td></tr>';
    ...
    if (check_allow('participants_change_status')) {
        ...
        if ($_REQUEST['status_id'] == '0') $hide = array();
        else $hide = array('0');
...
function participant__password_form_fields($new = false, $provided = false)
{
    $out = '';
    $out = '<tr><td>';
    if ($new) $out .= lang('new_password');
    else $out .= lang('password');
    $out .= '<br>';
    ...
    $out .= '</td></tr>
        <tr><td>';
    if ($new) $out .= lang('repeat_new_password');
    else $out .= lang('repeat_password');
    $out .= '<br>';
    if ($provided) $out .= '***' . lang('provided') . '***';
...
function participant_status__get_statuses()
{
    ...
    if (isset($preloaded_participant_statuses) && is_array($preloaded_participant_statuses) && count($preloaded_participant_statuses) > 0) {
        ...
        $result = or_query($query);
        while ($line = pdo_fetch_assoc($result)) {
            if ($line['content_type'] == 'participant_status_name') $field = 'name';
            else $field = 'error';
...
function participant__get_result_table_columns($list)
{
    ...
    if (isset($preloaded_result_table_columns[$list]) && is_array($preloaded_result_table_columns[$list])) return $preloaded_result_table_columns[$list];
        ...
        $result = or_query($query, $pars);
        $saved_cols = array();
        while ($line = pdo_fetch_assoc($result)) $saved_cols[$line['item_name']] = $line;
                ...
                case 'payment_budget':
                    if ($settings['enable_payment_module'] == 'y') {
                        ...
                        if (check_allow('payments_edit')) {
                            ...
                            $out .= '<td class="small">';
                            if (isset($payment_budgets[$p['payment_budget']])) $out .= $payment_budgets[$p['payment_budget']]['budget_name'];
                            else $out .= '-';
                            $out .= '</td>';
                        }
                    }
                ...
                case 'payment_type':
                    if ($settings['enable_payment_module'] == 'y') {
                        ...
                        if (check_allow('payments_edit')) {
                            ...
                            $out .= '<td class="small">';
                            if (isset($payment_types[$p['payment_type']])) $out .= $payment_types[$p['payment_type']];
                            else $out .= '-';
                            $out .= '</td>';
                        }
                    }
                ...
                case 'payment_amount':
                    if ($settings['enable_payment_module'] == 'y') {
                        if (check_allow('payments_edit')) {
                            ...
                            if ($p['payment_amt'] != '') $out .= $p['payment_amt'];
                            else $out .= '0.00';
            ...
            switch ($k) {
                ...
                case 'payment_budget':
                    if ($settings['enable_payment_module'] == 'y' && check_allow('payments_view')) {
                        $payment_budgets = payments__load_budgets();
                        if (isset($payment_budgets[$p['payment_budget']])) $row[] = $payment_budgets[$p['payment_budget']]['budget_name'];
                        else $row[] = '-';
                    }
                    break;
                case 'payment_type':
                    if ($settings['enable_payment_module'] == 'y' && check_allow('payments_view')) {
                        $payment_types = payments__load_paytypes();
                        if (isset($payment_types[$p['payment_type']])) $row[] = $payment_types[$p['payment_type']];
                        else $row[] = '-';
                    }
                    break;
                case 'payment_amount':
                    if ($settings['enable_payment_module'] == 'y' && check_allow('payments_view')) {
                        if ($p['payment_amt'] != '') $row[] = $p['payment_amt'];
                        else $row[] = '-';
                    }
                    break;
                case 'pstatus_id':
