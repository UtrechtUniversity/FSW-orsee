...
function experimentmail__send_mails_from_queue($number = 0, $type = "", $experiment_id = "", $session_id = "")
{
    ...
    foreach ($reminders as $mail) {
        ...
        $done = experimentmail__send_session_reminder_mail(
            $mail,
            $parts[$mail['mail_recipient']],
            $exps[$mail['experiment_id']],
            $sesss[$mail['session_id']],
            $reminder_text[$mail['experiment_id']][$tlang],
            $labs[$mail['session_id']][$tlang],
            $footers[$tlang]
        );
        if ($done) {
            $mails_sent++;
    ...
    foreach ($warnings as $mail) {
        $tlang = $parts[$mail['mail_recipient']]['language'];
        $done = experimentmail__send_noshow_warning_mail(
            $mail,
            $parts[$mail['mail_recipient']],
            $exps[$mail['experiment_id']],
            $sesss[$mail['session_id']],
    ...
    foreach ($invitations as $mail) {
        ...
        if ($exps[$mail['experiment_id']]['experiment_type'] == 'laboratory' && (!trim($slists[$mail['experiment_id']][$tlang]))) {
            ...
            $done = experimentmail__send_invitation_mail(
                $mail,
                $parts[$mail['mail_recipient']],
                $exps[$mail['experiment_id']],
                $inv_texts[$mail['experiment_id']][$tlang],
                $slists[$mail['experiment_id']][$tlang],
                $footers[$tlang]
            );
            if ($done) $mails_sent++;
        }
...
{
    $pars = array(
        ':participant_id' => $mail['mail_recipient'],
...
{
    if (!$maillang) $maillang = lang('lang');
    $mailtext = array();
