...
function experiment__load_experimenters()
{
    ...
    if (
        isset($preloaded_experimenters) && is_array($preloaded_experimenters)
...
function experiment__allowed($experiment_var)
{
    ...
    if ($experiment['access_restricted'] == 'y') {
        global $settings, $expadmindata;
        if (
            $settings['allow_experiment_restriction'] == 'y' &&
...
function experiment__experiments_format_alist($alist)
{
    ...
    if (!isset($roweven) || $roweven == true) $roweven = false;
    else $roweven = true;
    ...
    foreach ($sessions as $s) {
        if (isset($s['num_registered'])) $reg = $s['num_registered'];
        else $reg = 0;
...
function experiment__load_experimentclassnames()
{
    ...
    if (
        isset($preloaded_experimentclasses) && is_array($preloaded_experimentclasses)
...
function experiment__preload_experiments()
{
    ...
    if (
        isset($preloaded_experiments) && is_array($preloaded_experiments)
